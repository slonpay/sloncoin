...
#if defined(NDEBUG)
# error "Sloncoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xf35b901069d6f72abf3622f504f7873f147d6c6a85f6663f5dc0fd5892afa9d0");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Sloncoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Sloncoin Signed Message:\n";
...
{
    // Sloncoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Sloncoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Sloncoin: 1 days
static const int64 nTargetSpacing = 5 * 60; // Sloncoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Sloncoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Sloncoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x2e0eb54a27f34c248c25df09c6e9cc3aec309d64c6913768bbb31156683dd52b");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block d
        const char* pszTimestamp = "Doug Jones defeats Roy Moore in alabama senate state";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1528389360;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084606829;

        if (fTestNet)
        {
            block.nTime    = 1528389330;
            block.nNonce   = 385660901;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xbf3b94b29649564839a12b6d67488a2c317bb5e3882dd7a55a541e0f87d0b31a"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Sloncoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SloncoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SloncoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SloncoinMiner(CWallet *pwallet)
{
    printf("SloncoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("sloncoin-miner");
    ...
    try { loop {
        ...
        printf("Running SloncoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SloncoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SloncoinMiner, pwallet));
}
